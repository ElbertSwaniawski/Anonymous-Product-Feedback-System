 ...
 *
 * Example: ts-node scripts/create-fhevm-example.ts encrypted-voting ./my-voting-example
 */
...
const EXAMPLES_MAP: Record<string, ExampleConfig> = {
  'encrypted-voting': {
    ...
    description: 'A privacy-preserving voting system demonstrating encrypted storage and access control',
  },
  'public-voting': {
    contract: 'contracts/PublicVotingSystem.sol',
    test: 'test/PublicVotingSystem.ts',
    description: 'A reference implementation of a public voting system for comparison',
  },
};
...
function createExample(exampleName: string, outputDir: string): void {
  ...
  const templateContract = path.join(outputDir, 'contracts', 'ExampleContract.sol');
...
function main(): void {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    ...
    log('  ts-node scripts/create-fhevm-example.ts encrypted-voting ./my-encrypted-voting\n');
    process.exit(0);
  }
