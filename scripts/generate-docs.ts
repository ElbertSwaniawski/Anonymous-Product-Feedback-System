 ...
 *
 * Example: ts-node scripts/generate-docs.ts encrypted-voting --output docs/
 */
...
const EXAMPLES_CONFIG: Record<string, DocsConfig> = {
  'encrypted-voting': {
    ...
    description: 'A privacy-preserving voting system demonstrating encrypted storage, access control, and anonymous aggregation patterns.',
    contract: 'contracts/PrivateVotingSystem.sol',
    test: 'test/PrivateVotingSystem.ts',
    output: 'docs/encrypted-voting.md',
    category: 'Voting Systems',
  },
  'public-voting': {
    title: 'Public Voting System',
    description: 'A reference implementation of a public voting system for comparison with privacy-preserving alternatives.',
    contract: 'contracts/PublicVotingSystem.sol',
    test: 'test/PublicVotingSystem.ts',
    output: 'docs/public-voting.md',
    category: 'Voting Systems',
  },
};
...
function main(): void {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    ...
    log('  ts-node scripts/generate-docs.ts encrypted-voting');
